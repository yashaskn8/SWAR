...
export class AuthController {
  ...
  @ApiBody({
    schema: {
      ...
      required: ['email', 'organizationSlug', 'devicePublicId', 'password'],
      properties: {
        email: { type: 'string', format: 'email' },
        organizationSlug: { type: 'string', maxLength: 80 },
        devicePublicId: { type: 'string', maxLength: 128 },
        password: { type: 'string', format: 'password', writeOnly: true, maxLength: 1024 },
      },
    },
  })
  ...
  @ApiBody({
    schema: {
      ...
      properties: { refreshToken: { type: 'string', writeOnly: true, maxLength: 512 } },
    },
  })

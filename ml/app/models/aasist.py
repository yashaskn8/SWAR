"""Verified AASIST asynchronous deep spoof-evidence adapter."""

from __future__ import annotations

import gc
from pathlib import Path
from typing import Any

import numpy as np
import torch

from app.models.interfaces import ModelAdapter, ModelAdapterError, ModelErrorCode, ModelInput
from app.models.rawnet2 import _load_source
from app.models.registry import ModelSpec, verified_artifact_path


def repeat_pad(samples: np.ndarray, required_samples: int) -> np.ndarray:
    """Match the official AASIST evaluation repeat-padding rule without persistence."""

    if samples.ndim != 1 or samples.size == 0 or required_samples <= 0:
        raise ModelAdapterError(ModelErrorCode.INVALID_MODEL_INPUT)
    if samples.size >= required_samples:
        return samples[:required_samples].astype(np.float32, copy=True)
    repeats = required_samples // samples.size + 1
    return np.tile(samples, repeats)[:required_samples].astype(np.float32, copy=False)


class AasistAdapter(ModelAdapter):
    def __init__(self, spec: ModelSpec, checkpoint_root: Path, device: torch.device) -> None:
        super().__init__(spec.metadata())
        self._spec = spec
        self._checkpoint_root = checkpoint_root
        self._device = device
        self._model: torch.nn.Module | None = None

    def load(self) -> None:
        artifacts = {
            artifact.artifact_id: verified_artifact_path(self._checkpoint_root, artifact)
            for artifact in self._spec.artifacts
        }
        try:
            source = _load_source(artifacts["aasist-source"], "swar_verified_aasist")
            model = source.Model(self._spec.architecture).to(self._device)
            state = torch.load(
                artifacts[self._spec.checkpoint_artifact_id],
                map_location=self._device,
                weights_only=True,
            )
            model.load_state_dict(state, strict=True)
            model.eval()
        except Exception as error:
            raise ModelAdapterError(ModelErrorCode.MODEL_ARCHITECTURE_MISMATCH) from error
        self._model = model
        self._mark_ready()

    @staticmethod
    def bonafide_logit(output: object) -> float:
        if not isinstance(output, tuple) or len(output) != 2:
            raise ModelAdapterError(ModelErrorCode.MODEL_INFERENCE_FAILED)
        logits = output[1]
        if not isinstance(logits, torch.Tensor) or logits.shape != (1, 2):
            raise ModelAdapterError(ModelErrorCode.MODEL_INFERENCE_FAILED)
        return float(logits[0, 1].detach().cpu().item())

    def _infer_raw(self, model_input: ModelInput, **_: Any) -> float:
        if model_input.samples.size != 64_000:
            raise ModelAdapterError(ModelErrorCode.INVALID_MODEL_INPUT)
        if self._model is None:
            raise ModelAdapterError(ModelErrorCode.MODEL_NOT_READY)
        padded = repeat_pad(model_input.samples, self.metadata.input_samples)
        waveform = torch.from_numpy(padded.copy()).unsqueeze(0).to(self._device)
        padded.fill(0.0)
        with torch.inference_mode():
            output = self._model(waveform)
        score = self.bonafide_logit(output)
        del waveform, output
        return score

    def close(self) -> None:
        self._model = None
        gc.collect()
        if self._device.type == "cuda":
            torch.cuda.empty_cache()
        self._mark_closed()
